...
class Solution {
    ...
    public int threeSumMulti(int[] arr, int target) {
        ...
        // Initialize dp array
        for (int i = 0; i <= n; i++) {
            dp[i][0][0] = 1; // One way to select 0 elements to get a sum of 0
        }
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= target; j++) {
                for (int k = 0; k <= 3; k++) {
                    dp[i][j][k] = dp[i - 1][j][k]; // Exclude the current element
                    if (k > 0 && j >= arr[i - 1]) {
                        dp[i][j][k] = (dp[i][j][k] + dp[i - 1][j - arr[i - 1]][k - 1]) % MOD; // Include the current element
                    }
                }
            }
        }
